...
- **Consequences:** All clipboard updates are framed as structured `ClipboardMessage` Postcard envelopes on channel 1; loops and duplicates are suppressed at zero network bandwidth cost; platform adapters can be cleanly hooked into `ClipboardBackend`.
