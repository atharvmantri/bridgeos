...
## ADR-0015: Notification Mirroring — Privacy-First Design with Keyword & App-ID Blocklists
- **Date:** 2026-09-17
- **Status:** Accepted
- **Decision:** The `NotificationPolicy` blocks notification forwarding based on two independent criteria: (1) a statically compiled app-ID blocklist of known banking, authenticator, password manager, and medical apps; (2) a keyword scan of the notification title and body for known sensitive patterns (2FA/OTP codes, financial transactions, verification codes). Blocking occurs on both inbound and outbound paths. The engine provides three built-in policy profiles: `default` (blocks sensitive, passes all urgencies), `strict` (blocks sensitive + ongoing, minimum Normal urgency), and `permissive` (blocks sensitive, passes Low urgency). No ML or regex — plain substring matching on lowercased strings.
- **Reasoning:** Notification mirroring has a significant privacy surface. A banking OTP or "Verification Code: 492817" notification forwarded to a PC that can be physically observed by coworkers directly enables social engineering and account takeover. The cost of a false positive (notification not mirrored) is low — the user can glance at their phone. The cost of a false negative (2FA code or bank balance exposed on a PC screen) is high. Substring matching is deterministic, auditable by any contributor, and introduces no ML model supply chain risk. App-ID matching catches the most dangerous category (banking apps, authenticator apps) with negligible false positives. Full per-category configuration is deferred to a user settings layer (not yet implemented).
- **Alternatives Considered:** ML-based content classification (rejected: introduces model dependency, non-deterministic, not auditable); regex patterns (rejected: overkill for fixed known keyword sets, prone to catastrophic backtracking bugs); no privacy filtering (rejected: violates zero-trust security model and user trust expectations); user-defined allowlists only (rejected: too much friction for safe defaults; new users should not need to configure privacy manually on first use).
- **Consequences:** Some legitimate notifications may be suppressed if their content incidentally matches keyword patterns (e.g. an app legitimately titled "Security Code Scanner"). Users requiring full notification mirroring for specific apps can adjust the policy profile. This ADR must be revisited when a user settings UI is implemented.

---

## ADR-0016: Golden Protocol Interoperability Fixtures as the Android Wire Contract
- **Date:** 2026-09-17
- **Status:** Accepted
- **Decision:** Canonical binary fixtures for every BridgeOS protocol message type are generated and self-verified by the Rust test suite in `tests/interop/fixtures/`. These fixtures serve as the formal wire contract for all non-Rust BridgeOS implementations (initially the Kotlin Android client). Any Kotlin code claiming to implement BridgeOS protocol compatibility MUST decode these fixtures byte-exactly and produce matching outputs. The fixture generator test (`tests/integration/tests/interop_fixtures.rs`) runs on every `cargo test` invocation and fails if the Rust implementation diverges from the committed fixtures.
- **Reasoning:** Without a shared cross-language wire compatibility contract, it is impossible to verify that Kotlin deserialization correctly handles Postcard encoding edge cases, endianness, enum tag placement, or BRG1 framing. Discovering incompatibilities only at live TCP connection time would make debugging extremely painful. Generating fixtures from the authoritative Rust source eliminates ambiguity: Rust is the reference implementation; Android must match it exactly. Fixtures are committed to the repository so new agent sessions can implement Android support without running a live Rust process.
- **Alternatives Considered:** Hand-authored hex fixture tables in documentation (rejected: error-prone, not automatically verified, becomes stale); live TCP integration test between Rust and Kotlin processes in CI (valuable, not rejected, but deferred until a complete Kotlin implementation exists; fixtures provide the immediate first gate); JSON-based fixture files only (rejected: Postcard is binary, not JSON; JSON fixtures would not catch encoding bugs in binary payload paths).
- **Consequences:** Fixture files in `tests/interop/fixtures/` become a versioned, immutable contract. Protocol-breaking changes MUST regenerate fixtures AND produce a new ADR documenting the wire break. Android implementation passes acceptance when all fixture-based unit tests in the Kotlin `:core:test` suite succeed.
