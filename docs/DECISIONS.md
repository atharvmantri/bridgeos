...
- **Alternatives Considered:** TOFU (Trust-On-First-Use without SAS) - rejected as vulnerable to local network interception and impersonation; storing private keys in SQLite trust database - rejected due to blast radius and permission granularity issues; storing trust lists in flat JSON - rejected due to race conditions and lack of transactional atomicity.
